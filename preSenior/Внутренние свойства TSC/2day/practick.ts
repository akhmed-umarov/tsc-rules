...
function filterPhonesByDate(
	...
	const dataInitial: IMobilePhone[typeof key] = new Date(initial)
	return phones.filter(({manufactured} , index)=> { 
		return manufactured instanceof Date && manufactured > dataInitial
	}).map(el=>({...el , initialDate: initial}))
}
